...
// The type alias for collection of call records of callback function called by SystemUtils_ForEachDir.
struct CallRecord
{
    void* context;
    std::string baseDir;
...
bool operator==(const CallRecord& first, const CallRecord& second)
{
...
public:
    ...
    /**
     * @brief provides the instance to this text fixture object.
     *
    ...
    CallRecord CreateCallRecord(std::string subdir)
    {
     ...
     */
    std::vector<CallRecord> ExecuteForEachDirTestCase(std::vector<std::string> subdirs, std::string excludedSubDir, int expectedRetCode, bool shouldDeleteBaseDirBeforeExecuteTest = false)
    {
        ...
        std::sort(m_actual_foreach_callback_records.begin(), m_actual_foreach_callback_records.end(), std::less<CallRecord>());
     ...
     */
    void AddCallRecord(const CallRecord& call_record)
    {
     ...
     * @param call_record The call record to save.
     */
    static void AddCallRecord(void* instance, const CallRecord& call_record)
    {
...
private:
    ...
    std::vector<CallRecord> m_actual_foreach_callback_records;
};
...
static void ForEachDir_Callback(void* context, const char* baseDir, const char* subDir)
{
    TestCaseFixture::AddCallRecord(context, CallRecord{ context, baseDir, subDir });
}
...
TEST_CASE_METHOD(TestCaseFixture, "SystemUtils_ForEachDir")
{
    ...
    SECTION("Non-Empty Dir, no excludeDir should callback")
    {
        ...
        CHECK(actualCallRecords == std::vector<CallRecord>{
            CreateCallRecord(BaseDir_for_ForEachDir() + "subdir1"),
        });
    }
    ...
    SECTION("two subdirs, exclude the 1st one")
    {
        ...
        CHECK(actualCallRecords == std::vector<CallRecord>{
            CreateCallRecord(BaseDir_for_ForEachDir() + "subdir2"),
            CreateCallRecord(BaseDir_for_ForEachDir() + "subdir3"),
        });
    }
